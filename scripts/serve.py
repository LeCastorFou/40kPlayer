...
"""Lance le service web 40kPlayer : parties à deux joueurs (liens secrets) ou contre le bot, sauvegardées.
    ...
    python3 scripts/serve.py --data-dir ~/40k/parties --access-code secret
    python3 scripts/serve.py --vs-bot --side defender --attacker-list ec_mercurial_host_2000

Sur l'accueil : créer une partie (listes, joueurs humains ou bot), récupérer les liens des deux joueurs,
reprendre une partie en cours, importer une liste NewRecruit. Chaque partie est un fichier JSON du
...
def main(argv=None) -> int:
    ...
    p.add_argument("--access-code", help="code demandé pour créer une partie ou enregistrer une liste (défaut : FORTYK_ACCESS_CODE)")
    ...
    serve(host=args.host, port=args.port, open_browser=not args.no_browser, data_dir=args.data_dir, access_code=args.access_code, quick=quick)
    return 0
